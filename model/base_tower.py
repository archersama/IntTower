...
class BaseTower(nn.Module):
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            with tqdm(enumerate(train_loader), disable=verbose != 1) as t:
                for _, (x_train, y_train) in t:
                    ...
                    y_contras = y_train.to(self.device)
            ...
            self.writer.add_scalar("Loss/train", epoch_logs["loss"], epoch)
    ...
    def get_embedding(self, x, batch_size=256):
        model = self.eval()
        if isinstance(x, dict):
            x = [x[feature] for feature in self.feature_index]
        for i in range(len(x)):
        ...
        with torch.no_grad():
            for _, x_test in enumerate(test_loader):
                ...
                item_embedding =  model(x)[2].cpu().data.numpy()
                user_embedding = model(x)[1].cpu().data.numpy()

                # y_pred = model(x)[0].cpu().data.numpy()

                # y_pred = model(x).cpu().data.numpy()
                user_list.append(user_embedding)
                item_list.append(item_embedding)
                # pred_ans.append(y_pred)

        return np.concatenate(user_list).astype("float64"),np.concatenate(item_list).astype("float64"),
    ...
    def compile(self, optimizer, loss=None, metrics=None,lr = 0.01):
        ...
        # self.scheduler = CosineAnnealingWarmRestarts(self.optim, T_0=231, T_mult=1,
